...
Lombok 的 annotation 都是在編譯時才被轉換成 java code，因此早期在使用 Lombok 時，開發人員還要在 IDE 安裝套件才能正常使用 Lombok。時至今日，它已直接被整合進了 IntelliJ IDEA，不需額外安裝套件，只要引入 dependency 即可；但使用其他 IDE 的開發者就要比較麻煩一點了，可以參考官方的[安裝教學](https://projectlombok.org/setup/eclipse)。
...
`@EqualsAndHashCode`: 以所有 non-static 和 non-transient field 來實作 `equals()` 和 `hashCode()`。如果 class 不能夠良好的 override 這兩個 method，一旦放入 HashSet 或當作 HashMap 的 key 值時，[可能會引發 memory leak](https://www.baeldung.com/java-memory-leaks#3-improper-equals-and-hashcode-implementations)。
...
### **@Builder 的危險性**
這是 [Builder Pattern](https://en.wikipedia.org/wiki/Builder_pattern) 的語法糖，讓我們能優雅的建立一個 instance：
...
Student student = Student.builder()
    ...
    .build();
```
這在建立物件時還挺方便的，尤其是在測試裡。不過，它彈性優雅的風格相對帶來副作用。例如 field 太多時，容易導致開發者忘了賦值而創建出一個**不完整**的 instance，最後可能導致難以發現的 bug。可透過重構手段，**讓 class 變小一點**，減少開發者忘記賦值的機率。
...
在 class 標註 `@Accessors(fluent=true)` 後就能以類似 fluent-style 的方式優雅的建立 instance。另外，將必要的 field 加上 `final`，使用者就必須在 constructor 裡填入它：
...
可能有些人會問我：用 `@Builder` 不是也差不多嗎？但我的經驗是，因為我有時會用 Factory 來建立物件，這時如果在工廠裡再用 builder pattern 反而有點多此一舉，此外，我的方式不需要額外呼叫 `builder()`, `build()`，而且也不必透過 Lombok 產生整個 Builder Pattern，只有 setter 而已，變得單純許多。
