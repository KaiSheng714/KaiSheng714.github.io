...
因此，除了不要實作那些現在用不到的程式碼，我們也要學習斷捨離的精神，盡量刪除那些用不到的程式碼，保持整個專案的乾淨 (Clean Code)。YAGNI 原則不僅加快了開發速度，且還允許保持 code base 簡單、更容易維護。

## **如何不違反 YAGNI 原則**
 
首先要解決眼前的問題、設計要足夠符合 user 的需求，以**簡單**為出發點，並自我檢視 :
 
**我要寫的這個 class/method/function，現在真的需要嗎 ?**
...
## **後記**
不要實作那些現在用不到的程式碼，沒有什麼比**不寫程式碼**更能節省時間了。
