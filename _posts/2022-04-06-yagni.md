...
如果你設計的是給其他人使用的 API、函式庫，或是 DB schema 時，就需要多花一點心力考慮未來可能會發生的狀況、未來可能會用到的功能。以開發 library 為例，你的 client 們無法每次都為了更新 library 而讓他們的 app 也得重新修改、compile、上架。此時 SOLID 之一的開放封閉原則 **Open-Closed Principle (OCP)** 就得派上用場。
