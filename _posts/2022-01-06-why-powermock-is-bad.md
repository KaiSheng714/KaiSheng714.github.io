...
除了寫大範圍的整合測試，另一個可行的方案就是使用 PowerMock。承上面所說的：對於 PowerMock 來說，production code 的實作細節一覽無遺。因此開發者可以透過 PowerMock 的各種 API 去控制待測類別的行為，先寫 test case，讓 legacy code 有保護與基本驗證方法後，就能讓開發者更有信心、大膽的重構。
