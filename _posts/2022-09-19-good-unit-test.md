...
其實 `getPermission` 是不用 `verify` 的。我們應該要想清楚這個測試在驗證什麼？什麼才是我們最關心的？通常就是這個 method 有沒有 side-effect，有 side-effect 的行為我們才需要 verify。這是個**取捨(trade-off)**，如果程式每個路徑都去 verify，確實最有可能會抓到 bug、單元測試保護力最高，但這樣卻更容易導致讓測試變得太敏感，就像剛剛小明的例子一樣，就連重構也可能導致測試失敗。因此比較好的做法是只 verify 有 side-effect 的行為。
