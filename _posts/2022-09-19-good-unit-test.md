...
單元測試已是軟體工程師必備的技能，但在我的經驗中，有些人寫的單元測試看起來好像有那麼一回事，但實際上並沒測到重點，而且還很容易因為重構而導致測試失敗，可說是為了測試而測試。如果像這樣的不良測試持續增加，不僅沒有帶來好處，還使專案更不穩健，因此遵循 Best Practice 是很重要的，本文將介紹 Java unit test 最佳實踐的案例與教學。
...
如果我們驗證的時候過度指定，就會讓測試程式變得**很敏感**，也容易出現不準確的問題。例如有一個說早安的程式，測試如下：
