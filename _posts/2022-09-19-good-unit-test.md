...
我們可以看到過度依賴 mock framework 了。太多假的行為，根本不曉得 transaction 有沒有真正成功，即測試結果可能不準確。這個看起來好像有在測，但這個測試給予我們的信心卻很低，因為講白了就是**太假了**。因此這個測試案例可能無法測出真實的問題。解決辦法就是不要寫單元測試，應該將它昇華到**整合測試**階段，雖然整合測試的成本較高，但結果也會比較貼近真實、有價值。
