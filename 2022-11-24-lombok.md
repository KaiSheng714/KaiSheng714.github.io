...
Project Lombok 是很實用且被廣泛運用的語法糖 library，它可以減少許多樣板程式例如 getter, setter 等，我們只需幾個簡單的 annotation，例如 @Data，就能幫助我們省下大量重複的工作，讓開發者更專注於關鍵的邏輯，進而提高開發效率。但我認為 Lombok 也存在缺點，若過度使用 Lombok 可能導致問題，或使程式更加複雜。
...
另一個不錯的選擇是`@Value`，它和`@Data`幾乎一樣，差別在於它去除了 `@Setter`，可用於建立 immutable object。
因為Lombok使代碼開發非常簡便，這就使得

Lombok 雖然方便好用，減少開發負擔，也正因如此就導致部分開發者過度依賴了它。因此在使用 Lombok 之餘，應關心以下幾個議題：
...
### **隱藏複雜性**
之前提過，[Constructor Injection](https://kaisheng714.github.io/articles/analyzing-dependency-injection-patterns-in-spring) 暴露了 constructor 中含有過多的參數 (Long Parameter List) 的壞味道，正常的開發者看到過多參數肯定會不舒服，表示它需要被重構，盡可能使它符合 SRP。
...
這會讓開發者看不到巨大的 constructor，即使注入很多個 dependency 看起來好像也沒關係。這種 Lombok + constructor injection 的組合用法反而隱藏了複雜性。但如果團隊成員都有程式壞味道的意識、熟悉重構的手段，像用法可以使寫法更簡潔，我覺得是可以允許的。
...
## **我常用的方式**
綜上所述，我認為 Lombok 還是最適合用於 Entity/Model。Lombok 這樣用就足以應付大部分的狀況了：
