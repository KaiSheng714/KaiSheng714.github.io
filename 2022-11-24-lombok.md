...
`@Data` 很適合用於 DTO，因為它只保存資料，沒有邏輯，基本上可以無腦使用也不會產生太多問題。另一個不錯的選擇是`@Value`，它和`@Data`幾乎一樣，差別在於它去除了 `@Setter`，可用於建立 immutable object。Lombok 雖然方便好用，但應該關心以下幾個議題：
...
### **Tell, Don't Ask.**
既然 Lombok 這麼方便就產生了 getter，就容易讓人忽略了 **Tell, Don't Ask** 原則的建議。不適當的詢問物件的屬性，導致它原本不會被暴露的內部狀態都洩漏出去了，破壞了封裝 (Encapsulation)。

Tell, Don't Ask 原則提醒開發者：所謂 OOP 就是將資料與操作該資料的 method 綁在一起。與其在 service 層用了一堆 getter 然後進行邏輯運算，不如直接請它做。必要時可在指定的 field 個別標記 `@Getter` 或 `@Getter(AccessLevel.NONE)`，來決定要暴露哪個 field。

延伸閱讀: [軟體設計原則 Tell, Don't Ask](/articles/tell-dont-ask)
