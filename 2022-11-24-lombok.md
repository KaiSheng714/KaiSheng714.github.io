...
### **隱藏複雜性**
之前提過，[Constructor Injection](https://kaisheng714.github.io/articles/analyzing-dependency-injection-patterns-in-spring) 這種方式暴露了 constructor 中含有過多的參數 (Long Parameter List)，這是個很好的臭味偵測器，這就表示我們需要想辦法重構它，盡可能使它符合單一職責原則 (Single Responsibility Principle)，

我看過這樣的用法：使用 Lombok `@AllArgsConstructor` 一行建立 constructor，取代 Spring `@Autowired`：
...
這會讓開發者看不到巨大的 constructor，因此變相的隱藏了複雜性。
