...
### **容易被驗證**
程式對於任何一項操作都要能產生預期的輸出，而輸出表現形式通常是可預期的回傳值、內部狀態、外在行為。不管是以什麼方式表現，至少都要有跡可循。若能很容易的驗證程式結果是否符合預期，則才能降低測試的難度，例如只用 `assertEqual()`, `assertTrue()` 等簡單驗證方法就能得出測試結果。如果待測程式的回傳值、內部狀態、外在行為不容易被驗證，那麼這個程式就不容易測試。
...
TDD 是一種**先從使用者角度寫測試，再回頭撰寫產品程式碼的開發手法**。因為 TDD 讓開發者換位思考，從使用者的角度出發，就更容易了解到該怎麼設計才能讓 class / API 更易用。為了先寫出測試，開發者就必須先去思考如何進行測試，他必須了解需求、必須逐步解構需求成一個個單純、小的 test case。若熟練 TDD 技術，就能解決大部分可測試性低的問題。
